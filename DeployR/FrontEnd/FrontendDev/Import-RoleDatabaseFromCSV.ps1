...
function Import-RoleDatabaseFromCSV {
    param(
...
function Convert-ToMacList {
    ...
    if (-not $rows -or $rows.Count -eq 0) {
        Write-Host "No rows found in CSV. Exiting."; return @()
    }
...
foreach ($r in $rows) {
    ...
    # Exclude devices in configured categories (case-insensitive)
    $skip = $false
    foreach ($ex in $ExcludeCategories) { if ($ex -and ($category.ToString().Trim().ToLower() -eq $ex.ToString().Trim().ToLower())) { $skip = $true; break } }
    if ($skip) { continue }
    ...
    # Build index and merge with counts
    $index = @{}
    ...
    if ($DryRun) {
        Write-Host "DryRun: Processed $($rows.Count) CSV rows; Existing=$existingCount; Final=$finalCount; Added=$added"
        return $merged
    }
